...
class ControladorLiquidacionPedidoVacaciones extends ControladorGeneral 
{
	...
	public function agregar($datos){
		try {
			...
			$parametros = array("idliquidacion" => $idLiquidacion[0]['id'], "idpedidovacaciones" => $datos['idpedidovacaciones']);
			$resualtado = $this->refControladorPersistencia->ejecutarSentencia(DbSentencias::AGREGAR_LIQUIDACIONPEDIDOVACACIONES, $parametros);
	...
	public function eliminar($datos){
		try {
			...
			$resultado = $this->refControladorPersistencia->ejecutarSentencia(DbSentencias::ELIMINAR_LIQUIDACIONPEDIDOVACACIONES, $parametros);
	...
	public function buscar($datos){
		try {
			...
			$resultado = $this->refControladorPersistencia->ejecutarSentencia(DbSentencias::BUSCAR_LIQUIDACIONPEDIDOVACACIONES, $parametros);
